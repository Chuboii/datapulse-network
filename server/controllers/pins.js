...
export const createUserPin = async (req, res, next) => {
  try {
    ...
    if (isPinCreated.length > 0)
      return next(createError(403, "You are not allowed to create a new pin"));
...
export const loginUserPin = async (req, res, next) => {
  try {
    ...
    if (userId === "65f8246072651dd963539ee5")
      return next(createError(401, "user is banned"));
    ...
    if (getPin.length === 0)
      return next(createError(403, "You have not created a pin"));
